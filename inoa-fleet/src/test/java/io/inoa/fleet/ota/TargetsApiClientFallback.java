...
public class TargetsApiClientFallback implements TargetsApiClient {
	...
	public HttpResponse<List<TargetsCreationResponsePartVO>> createTargets(
		...
		for (TargetsCreationRequestPartVO targetsCreationRequest : targetsCreationRequestPartVO) {
			...
			responses.add(new TargetsCreationResponsePartVO().controllerId(targetsCreationRequest.getControllerId())
					.name(targetsCreationRequest.getName()).securityToken(targetsCreationRequest.getSecurityToken()));
		}
	...
	public HttpResponse<TargetSearchResponseItemVO> getTarget(@NotNull String targetId) {
		return targets.containsKey(targetId) ? HttpResponse.status(HttpStatus.OK)
				.body(new TargetSearchResponseItemVO().controllerId(targets.get(targetId).getControllerId())
						.name(targets.get(targetId).getName()).securityToken(targets.get(targetId).getSecurityToken()))
				: HttpResponse.notFound();
	}

	@Override
	public HttpResponse<TargetsSearchResponseVO> getTargets(Integer limit, String sort, Integer offset, String query) {
		var searchResultItems = targets.values().stream()
				.map(targetsCreationRequestPartVO -> new TargetSearchResponseItemVO()
				...
				.toList();
		return HttpResponse.status(HttpStatus.OK).body(new TargetsSearchResponseVO().content(searchResultItems));
	}
}
