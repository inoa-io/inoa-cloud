 ...
 */
@MicronautTest
public class LogEventListenerTest extends AbstractTest {
	...
	void success() {
		...
		send(CloudEventTypeVO.IO_INOA_LOG_EMITTED_VALUE, tenantId, gatewayId,
				Map.of("tag", "METERING", "level", 3, "time", 123456789, "msg", "OK"));
		assertEquals(countSuccessBefore + 1, metrics.counterSuccess(tenantId).count(), "metrics success");
		assertEquals(countIgnoreBefore, metrics.counterIgnore(tenantId).count(), "metrics ignore");
		assertEquals(counterFailMessageReadBefore, metrics.counterFailMessageRead(tenantId).count(), "metrics fail");
	...
	void illegalPayload() {
		...
		assertEquals(countIgnoreBefore, metrics.counterIgnore(tenantId).count(), "metrics ignore");
		assertEquals(counterFailMessageReadBefore + 1, metrics.counterFailMessageRead(tenantId).count(),
				"metrics fail");
