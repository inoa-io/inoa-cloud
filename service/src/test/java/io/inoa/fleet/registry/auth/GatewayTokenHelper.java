...
public class GatewayTokenHelper {
	...
	public String bearer(Gateway gateway) {
		return bearer(
				data.findCredentials(gateway).stream().findFirst().orElse(data.credentialPSK(gateway)));
	}
	...
	@SneakyThrows
	public String token(
			String gatewayId, byte[] secret, Consumer<JWTClaimsSet.Builder> claimsManipulator) {
