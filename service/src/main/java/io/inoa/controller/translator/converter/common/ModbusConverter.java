...
public class ModbusConverter extends CommonConverter {
  ...
  public static final String COUNTER_INGORE_FUNCTION_CODE =
      "translator_modbus_ignore_function_code";
   ...
   */
  private static final Set<Integer> FUNCTION_CODE_EXCEPTION =
      Set.of(129, 130, 131, 132, 133, 134, 143, 144);
  ...
  public ModbusConverter(TranslatorProperties properties, MeterRegistry meterRegistry) {
  ...
  public Stream<TelemetryVO> convert(TelemetryRawVO raw, String type, String sensor) {
    ...
    if (functionCode != 3) {
      if (FUNCTION_CODE_EXCEPTION.contains(functionCode)) {
        ...
        log.info(
            "Retrieved modbus error message (functionCode {}) with error {}",
            functionCodeHex,
            error);
        increment(type, COUNTER_INGORE_ERROR_CODE);
      } else {
        log.info(
            "Retrieved invalid modbus message (functionCode {}): {}", functionCodeHex, hexString);
    ...
    var hexLength = hexString.length() - 4;
    if (hexLength < dataEndIndex) {
      log.info(
          "Retrieved invalid modbus message (data.length {} < byteCount {}): {}",
          (hexLength - 6) / 2,
          (dataEndIndex - 6) / 2,
          hexString);
    ...
    log.trace(
        "Modbus with slaveId {}, functionCode {} has value: {}",
        slaveIdHex,
        functionCodeHex,
        value);
