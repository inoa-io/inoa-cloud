...
public class NumberConverter extends CommonConverter {
	...
	public Stream<TelemetryVO> convert(TelemetryRawVO raw, String type, String sensor) {
		...
		if (properties.isPresent() && properties.get().getConfig() != null && !properties.get().getConfig().isEmpty()) {
			...
			if (config.containsKey(CONFIG_KEY_FACTOR)) {
				try {
					...
					log.debug("Failed to parse factor for type {}:  {}.", type, config.get("factor").toString());
					return Stream.empty();
				}
			}
		}
