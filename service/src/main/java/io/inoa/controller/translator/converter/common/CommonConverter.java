...
import io.inoa.controller.translator.converter.AbstractConverter;
...
public abstract class CommonConverter extends AbstractConverter {
  ...
  TelemetryVO convert(String type, String sensor, Double value) {
    var telemetry = new TelemetryVO().deviceType(type).sensor(sensor).value(value);
    get(type, sensor)
        .ifPresent(
            sensorProperties -> {
              ...
              sensorProperties
                  .getModifier()
                  .ifPresent(modifier -> telemetry.setValue(modifier * value));
            });
    return telemetry;
  }

  Optional<TranslatorProperties.SensorProperties> get(String type, String sensor) {
    return properties.getTypes().stream()
        ...
        .filter(
            sensorProperties ->
                Objects.equals(sensorProperties.getName(), sensor)
  ...
  void increment(String type, String counter) {
    counters
        .computeIfAbsent(type + counter, string -> meterRegistry.counter(counter, "type", type))
        .increment();
  }
}
