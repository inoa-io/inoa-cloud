...
public class TranslateService {
	...
	public List<TelemetryVO> translate(String tenantId, String gatewayId, TelemetryRawVO raw) {
		...
		var messages = converter
				.get()
				.convert(raw, deviceType, sensor)
				.peek(
						target -> fillMessage(raw, target, tenantId, gatewayId, deviceType, deviceId, sensor))
	...
	private boolean isInvalid(TelemetryVO vo) {
		...
		if (old == null) {
			...
			if (diffTime.isNegative()) {
				log.warn(
						"URN {} got out of order value (old {}, new {}), skip validation.",
						urn,
						oldTime,
						newTime);
				return true;
			...
			if (diffValue < 0) {
				log.warn(
						"URN {} had wh decreased from {} to {} Wh (diff {} Wh).",
						urn,
						oldValue,
						newValue,
						diffValue);
				...
				if (increase > increaseThreshold) {
					log.warn(
							"URN {} increased by {} Wh ({} Wh in {}), skip value bigger than thresold {} Wh.",
							urn,
							increase,
							diffValue,
							diffTime,
