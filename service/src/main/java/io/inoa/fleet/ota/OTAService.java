...
/**
 * Service for handling OTA related stuff. Decouples Hawkbit API from INOA
 * Fleet.
 */
@Slf4j
@Singleton
...
public class OTAService {
	...
	public boolean checkGatewayRegistered(String gatewayId) {
		...
		default -> {
			log.error("Unable to check if gateway is known to Hawkbit. Got status code {}: {}", response.code(),
					response.body());
	...
	public void registerGateway(String gatewayId, String psk) {
		var response = targetsApiClient.createTargets(Collections.singletonList(
				new TargetsCreationRequestPartVO().controllerId(gatewayId).name(gatewayId).securityToken(psk)));
		...
		default -> {
			log.error("Unable to create gateway with ID {} in Hawkbit. Got status code {}: {}", gatewayId,
