...
public class TenantsController implements TenantsApi {
  ...
  /**
   * Name of the default tenant that is already present in the database and cannot be deleted.
   * Reason is to have a tenant to which new gateways are configured at self registration.
   */
  ...
  public HttpResponse<TenantVO> createTenant(TenantCreateVO tenantCreateVO) {
    ...
    Tenant tenant =
        repository.save(
            new Tenant()
                .setTenantId(tenantCreateVO.getTenantId())
                .setName(tenantCreateVO.getName())
                .setEnabled(tenantCreateVO.getEnabled())
  ...
  @Override
  public HttpResponse<TenantVO> updateTenant(
      @NonNull String tenantId, @NonNull TenantUpdateVO tenantUpdateVO) {
    var changed = false;
    ...
    if (tenantUpdateVO.getName() != null) {
      if (updatedTenant.getName().equals(tenantUpdateVO.getName())) {
        ...
        log.info(
            "Group {}: updated name to {}.", updatedTenant.getName(), tenantUpdateVO.getName());
        changed = true;
    ...
    if (tenantUpdateVO.getEnabled() != null) {
      if (updatedTenant.getEnabled().equals(tenantUpdateVO.getEnabled())) {
        log.trace(
            "Tenant {}: skip update of enabled flag because not changed.", updatedTenant.getName());
      } else {
        log.info(
            "Group {}: updated enabled flag to {}.",
            updatedTenant.getName(),
            tenantUpdateVO.getEnabled());
        changed = true;
    ...
    if (tenantUpdateVO.getGatewayIdPattern() != null) {
      if (updatedTenant.getGatewayIdPattern().equals(tenantUpdateVO.getGatewayIdPattern())) {
        log.trace(
            "Tenant {}: skip update of gateway ID pattern because not changed.",
            updatedTenant.getName());
      } else {
        // Check Id Pattern
        checkGatewayIdPattern(tenantUpdateVO.getGatewayIdPattern());
        log.info(
            "Group {}: updated gateway ID pattern to {}.",
            updatedTenant.getName(),
  ...
  public HttpResponse<Object> deleteTenant(@NonNull String tenantId) {
    ...
    if (DEFAULT_TENANT_ID.equals(tenantId)) {
      throw new HttpStatusException(
          HttpStatus.BAD_REQUEST, "The default tenant cannot be deleted.");
    }
  ...
  private void checkGatewayIdPattern(String pattern) {
    try {
      ...
      throw new HttpStatusException(
          HttpStatus.BAD_REQUEST, "Invalid pattern for gateway ID: " + e.getLocalizedMessage());
    }
  }
}
