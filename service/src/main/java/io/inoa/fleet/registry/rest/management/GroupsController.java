...
public class GroupsController extends AbstractManagementController implements GroupsApi {
	...
	public HttpResponse<List<GroupVO>> findGroups() {
		return HttpResponse.ok(
				mapper.toGroups(repository.findByTenantInListOrderByName(security.getGrantedTenants())));
	}

	@Override
	public HttpResponse<GroupVO> findGroup(
			@NonNull UUID groupId, @NonNull Optional<String> tenantId) {
		return HttpResponse.ok(mapper.toGroup(getGroup(groupId, tenantId)));
	}

	@Override
	public HttpResponse<GroupVO> createGroup(
			@Valid GroupCreateVO vo, @NonNull Optional<String> tenantId) {
