...
public abstract class AbstractManagementController {
	...
	/**
	 * If an issuer uses the API with multiple tenants granted, a specific tenant the action should
	 * apply to, has to be given. Otherwise, a 401 is thrown. If the issuer has only one tenant grant,
	 * no specific tenant has to be given.
	 *
	...
	protected Tenant resolveAmbiguousTenant(List<Tenant> grantedTenants, Optional<String> tenantId) {
		...
		if (tenantId.isPresent()) {
			var specificTenant = grantedTenants.stream()
					.filter(tenant -> tenant.getTenantId().equals(tenantId.get()))
					.findFirst();
			if (specificTenant.isEmpty()) {
				throw new HttpStatusException(
						HttpStatus.UNAUTHORIZED, "Tenant " + tenantId.get() + " not granted.");
			}
			return specificTenant.get();
		} else {
			if (grantedTenants.size() > 1) {
				throw new HttpStatusException(
						HttpStatus.BAD_REQUEST, "Ambiguous tenant. Please specify tenant as query parameter.");
			}
