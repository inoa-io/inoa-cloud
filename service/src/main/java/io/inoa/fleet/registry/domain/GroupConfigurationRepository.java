...
public interface GroupConfigurationRepository extends GenericRepository<GroupConfiguration, Long> {
  ...
  @Query(
      "UPDATE group_configuration SET value = :value WHERE group_id = :groupId AND definition_id ="
          + " :definitionId")
  void update(Long groupId, Long definitionId, String value);

  @Query(
      "DELETE FROM group_configuration WHERE group_id = :groupId AND definition_id = :definitionId")
