...
import io.inoa.rest.PropertiesApi;
...
public class PropertiesController implements PropertiesApi {
            ...
            Stream.concat(
                    ...
                    body.entrySet().stream()
                        .map(
                            entry ->
                                updatedProperty(
                                    gateway, properties, entry.getKey(), entry.getValue())))
                .toList()));
  }
  ...
  public HttpResponse<Object> setProperty(String key, String newValue) {
    var gateway = security.getGateway();
    var properties =
        repository.findByGatewayAndKey(gateway, key).stream().collect(Collectors.toList());
  ...
  private GatewayProperty updatedProperty(
      Gateway gateway, List<GatewayProperty> properties, String key, String newValue) {

    var optionalProperty =
        properties.stream().filter(property -> property.getKey().equals(key)).findAny();
    if (optionalProperty.isEmpty()) {
