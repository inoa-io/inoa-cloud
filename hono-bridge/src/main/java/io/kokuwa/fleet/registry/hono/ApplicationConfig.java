...
public class ApplicationConfig {
	 ...
	 * <p>
	 * The Tracer will be resolved by means of a Java service lookup. If no tracer
	 * can be resolved this way, the {@code NoopTracer} is returned.
	 *
	 * @return The tracer.
	 */
	@Bean
	public Tracer tracer() {
		return Optional.ofNullable(TracerResolver.resolveTracer()).orElse(NoopTracerFactory.create());
	}
	...
	/**
	 * Gets general properties for configuring the Device Registry Spring Boot
	 * application.
	 *
	 * @return The properties.
	 */
	...
	/**
	 * Exposes a password encoder to use for encoding clear text passwords and for
	...
	/**
	 * Gets properties for configuring inoa.
	 *
	 * @return The properties.
	 */
	@Bean
	@ConfigurationProperties("hono.inoa")
	public InoaProperties inoaProperties() {
		return new InoaProperties();
	}
	...
	public MessagingClientProvider<EventSender> eventSenderProvider() {
		...
		if (downstreamSenderConfig().isHostConfigured()) {
			result.setClient(new ProtonBasedDownstreamSender(
					...
					SendMessageSampler.Factory.noop(), true, true));
		}
	...
	/**
	 * Exposes configuration properties for accessing the AMQP Messaging Network as
	 * a Spring bean.
	 *
	 * @return The properties.
	 */
	...
	/**
	 * Exposes configuration properties for a producer accessing the Kafka cluster
	 * as a Spring bean.
	 *
	 * @return The properties.
	 */
	 ...
	 * @throws IOException
	 *             if reading the SQL configuration fails.
	 */
	@Bean
	@Scope("prototype")
	@Profile(Profiles.PROFILE_REGISTRY_ADAPTER)
	public RegistrationService registrationService() throws IOException {
		...
		final RegistrationServiceImpl registrationService = new RegistrationServiceImpl(restTemplate(),
				inoaProperties(), tokenService());
		final EdgeDeviceAutoProvisioner edgeDeviceAutoProvisioner = new EdgeDeviceAutoProvisioner(vertx(),
				registrationManagementService(), eventSenderProvider(), autoProvisionerConfigProperties(), tracer());
	...
	public CredentialsService credentialsService() throws IOException {
		final CredentialsServiceImpl credentialsService = new CredentialsServiceImpl(restTemplate(), inoaProperties(),
		...
		final DeviceAndGatewayAutoProvisioner deviceAndGatewayAutoProvisioner = new DeviceAndGatewayAutoProvisioner(
				vertx(), registrationManagementService(), credentialsManagementService(), eventSenderProvider());
	...
	public TokenService tokenService() throws IOException {
		return new TokenService(new RestTemplate(), inoaProperties());
	}
	...
	public TenantService tenantService() throws IOException {
		return new TenantServiceImpl(restTemplate(), inoaProperties(), tokenService());
	}

	/**
	 * Provide a tenant information service, backed by the JDBC tenant management
	 * service instance.
	 *
	 * @return The bean instance.
	 * @throws IOException
	...
	public DeviceManagementService registrationManagementService() throws IOException {
		return new DeviceManagementServiceImpl(restTemplate(), inoaProperties(), tokenService());
	}
	...
	public CredentialsManagementService credentialsManagementService() throws IOException {
		return new CredentialsManagementServiceImpl(vertx(), passwordEncoder(), deviceRegistryServiceProperties());
	}
	...
	public TenantManagementService tenantManagementService() throws IOException {
		return new TenantManagementServiceImpl(restTemplate(), inoaProperties(), tokenService());
	}

	/**
	 * Creates a new instance of an AMQP 1.0 protocol handler for Hono's <em>Device
	 * Registration</em> API.
	 *
	...
	/**
	 * Creates a new instance of an AMQP 1.0 protocol handler for Hono's
	...
	/**
	 * Gets properties for configuring the HTTP based Device Registry Management
	 * endpoint.
	 *
	 * @return The properties.
	 */
	...
	/**
	 * Creates a new instance of an auth handler to provide basic authentication for
	 * the HTTP based Device Registry Management endpoint.
	 * <p>
	 * This method creates a {@link BasicAuthHandler} using the auth provider
	 * returned by {@link #authProvider()} if the property corresponding to
	 * {@link HttpServiceConfigProperties#isAuthenticationRequired()} is set to
	 * {@code true}.
	 *
	 * @param httpServiceConfigProperties
	 *            The properties for configuring the HTTP based device registry
	 *            management endpoint.
	 * @return The auth handler if the
	 *         {@link HttpServiceConfigProperties#isAuthenticationRequired()} is
	 *         {@code true} or {@code null} otherwise.
	 * @see <a href="https://vertx.io/docs/vertx-auth-jdbc/java/">JDBC Auth Provider
	 *      docs</a>
	 */
	@Bean
	@Scope("prototype")
	...
	/**
	 * Creates a new instance of an HTTP protocol handler for the <em>devices</em>
	 * resources of Hono's Device Registry Management API's.
	 *
	 * @return The handler.
	 * @throws IOException
	 *             if reading the SQL configuration fails.
	...
	/**
	 * Creates a new instance of an HTTP protocol handler for the
	 * <em>credentials</em> resources of Hono's Device Registry Management API's.
	 *
	 * @return The handler.
	 * @throws IOException
	 *             if reading the SQL configuration fails.
	...
	/**
	 * Creates a new instance of an HTTP protocol handler for the <em>tenants</em>
	 * resources of Hono's Device Registry Management API's.
	 *
	 * @return The handler.
	 * @throws IOException
