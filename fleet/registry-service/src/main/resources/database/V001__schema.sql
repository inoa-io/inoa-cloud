CREATE TABLE tenant (
	...
	CONSTRAINT chk_tenant_id CHECK (tenant_id ~ '^[a-z0-9\-]{4,30}$'),
	CONSTRAINT chk_tenant_name CHECK (length(name) >= 4)
);
CREATE TABLE "group" (
	id SERIAL NOT NULL,
...
CREATE TABLE credential (
	...
	CONSTRAINT uq_credential_name UNIQUE (gateway_id,name),
	CONSTRAINT chk_credential_type CHECK (type ~ '^PSK|RSA$'),
	CONSTRAINT chk_credential_name CHECK (name ~ '^[a-zA-Z0-9\-]{3,32}$'),
