INSERT INTO tenant (tenant_id,name,enabled,gateway_id_pattern,created,updated) VALUES ('inoa','INOA',true,'ISRL0[12]-[A-F0-9]{10}',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP);
...
INSERT INTO tenant_configuration (definition_id,value) VALUES ((SELECT id FROM configuration_definition WHERE tenant_id=(SELECT id FROM tenant WHERE tenant_id='inoa') AND KEY='mqtt.url'),'ssl://127.0.0.1:8883');
