...
public record TelemetryRawVO(
    ...
    @NotNull @JsonFormat(without = JsonFormat.Feature.READ_DATE_TIMESTAMPS_AS_NANOSECONDS)
        Instant timestamp,
    @NotNull byte[] value,
    Map<String, String> ext) {

  public static final String URN_REGEX =
      "urn"
          + ":(?<deviceType>[a-zA-Z0-9\\-]{2,32})"
          + ":(?<deviceId>[a-zA-Z0-9\\-]{2,36})"
          + ":(?<sensor>[a-zA-Z0-9_\\-\\:*]{2,64})";
  public static final java.util.regex.Pattern URN_PATTERN =
      java.util.regex.Pattern.compile(URN_REGEX);
